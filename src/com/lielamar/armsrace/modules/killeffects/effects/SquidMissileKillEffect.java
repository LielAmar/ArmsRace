	...
	public void playKillEffect(Main main, Location loc, Player victim, Player killer) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if(counter <= 0) {
					squid.remove();
					squid.getWorld().playSound(squid.getLocation(), Sound.FIREWORK_LARGE_BLAST2, 1.0F, 4.0F);
					this.cancel();
					return;
				}
				squid.setVelocity(new Vector(0, 0.3, 0));
				squid.getWorld().playSound(squid.getLocation(), Sound.ITEM_PICKUP, 1.0F, 4.0F);
