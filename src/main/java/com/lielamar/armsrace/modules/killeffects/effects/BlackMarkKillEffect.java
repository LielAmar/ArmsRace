...
public class BlackMarkKillEffect implements KillEffect {
	...
	public void playKillEffect(Main main, Location loc, Player victim, Player killer) {
		...
		loc.getWorld().playSound(loc, XSound.ENTITY_GENERIC_EXPLODE.get(), 1.0F, 4.0F);
		...
		for (Player pl : Bukkit.getOnlinePlayers()) {
			...
			ParticleUtils.sendParticle(pl, "SMOKE_LARGE", loc1, td, 1);
			ParticleUtils.sendParticle(pl, "SMOKE_LARGE", loc2, td, 1);
			ParticleUtils.sendParticle(pl, "SMOKE_LARGE", loc3, td, 1);
			ParticleUtils.sendParticle(pl, "SMOKE_LARGE", loc4, td, 1);
			ParticleUtils.sendParticle(pl, "SMOKE_LARGE", loc5, td, 1);
		}
	}
}
