	...
	public void playKillEffect(Main main, Location loc, Player victim, Player killer) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (counter <= 0) {
					squid.remove();
					squid.getWorld().playSound(squid.getLocation(), XSound.ENTITY_FIREWORK_ROCKET_LARGE_BLAST_FAR.get(), 1.0F, 4.0F);
					this.cancel();
					return;
				}
				squid.setVelocity(new Vector(0, 0.3, 0));
				squid.getWorld().playSound(squid.getLocation(), XSound.ENTITY_ITEM_PICKUP.get(), 1.0F, 4.0F);

				if (counter % 10 == 0) {
					for (Player pl : Bukkit.getOnlinePlayers()) {
						ParticleUtils.sendParticle(pl, "FLAME", squid.getLocation(), null, 1);
					}
				}
			}
